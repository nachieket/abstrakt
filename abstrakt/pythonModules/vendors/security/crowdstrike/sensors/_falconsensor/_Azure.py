...
class _Azure(_CrowdStrikeSensors):
  ...
  def get_az_service_principal_id(self, sp_name: str) -> str | None:
    try:
      command: str = f'az ad sp list --display-name {sp_name} --query "[].appId" --output tsv'
  ...
  def check_service_principal_exists(self, sp_name):
    command = f'az ad sp list --display-name {sp_name} --query "[].{{id:appId}}" --output tsv'
  ...
  def get_azure_image_tag(self, registry: str, registry_type: str, repository: str, image_tag: str,
    ...
    if registry_type == 'acr' and self.check_acr_registry_exists(registry=registry, acr_sub_id=acr_sub_id,
                                                                 acr_rg=acr_rg):
      # if not self.check_acr_repository_exists(acr_sub_id=acr_sub_id, acr_rg=acr_rg, registry=registry,
      #                                         repository=repository):
      #   pass

      # self.login_to_acr(acr=registry)
