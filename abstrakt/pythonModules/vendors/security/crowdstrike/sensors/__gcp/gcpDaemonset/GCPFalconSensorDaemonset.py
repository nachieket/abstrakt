...
from abstrakt.pythonModules.vendors.security.crowdstrike.sensors.__gcp.GCPFalconSensor import GCPFalconSensor
...
class GCPFalconSensorDaemonset(GCPFalconSensor):
  ...
  def execute_default_app(self):
    try:
      with MultiThreading() as mt:
        mt.run_with_progress_indicator(self.default_app_thread, 1, 300)
  ...
  def execute_helm_chart(self):
    try:
      with MultiThreading() as mt:
        mt.run_with_progress_indicator(self.daemonset_thread, 1, 300)
    except Exception as e:
      self.logger.error(f'Error in function {inspect.currentframe().f_back.f_code.co_name}')
      self.logger.error(f'Error: {e}')
      return False
...
class GCPDaemonsetKAC(GCPFalconSensor):
  ...
  def deploy_falcon_kac(self):
    ...
    try:
      with MultiThreading() as mt:
        mt.run_with_progress_indicator(self.gcp_daemonset_kac_thread, 1, 300)
