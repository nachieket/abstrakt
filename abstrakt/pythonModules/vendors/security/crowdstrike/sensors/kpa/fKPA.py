...
class FalconKPA(CrowdStrike):
  ...
  def execute_kpa_installation_process(self):
    falcon_api_base_url, falcon_cloud_region = self.get_base_url_and_region()

    if access_token := self.get_api_access_token(falcon_api_base_url) and falcon_api_base_url and falcon_cloud_region:
      try:
